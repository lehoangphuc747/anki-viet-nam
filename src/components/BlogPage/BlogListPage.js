  ...
  const getFilteredItems = () => {
    ...
    return items.filter(({ content: { frontMatter } }) => {
      ...
      if (activeSubCategory) {
        return (
          category === activeSubCategory.label ||
      ...
      return categoryConfig.subCategories.some(
        (sub) => category === sub.label || tags.includes(sub.tag)
      );
    });
  };
  ...
  return (
    <Layout title="Tất cả bộ thẻ" description="Các bộ thẻ Anki được chia sẻ bởi cộng đồng">
      <div className={styles.main}>
        <h1 className={styles.title}>Chào mừng bạn đến với Anki Việt Nam 👋</h1>
        <p className={styles.subtitle}>Dưới đây là các bộ thẻ đã chia sẻ của cộng đồng!</p>
              ...
              {categories[activeTab].subCategories.map((subCategory) => (
                <button
                  ...
                  className={`${styles.subCategoryLink} ${
                    activeSubCategory?.tag === subCategory.tag ? styles.active : ''
                  }`}
